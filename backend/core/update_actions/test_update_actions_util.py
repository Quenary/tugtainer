...
def _patch_session(
    ...
    mocker.patch(
        "backend.core.update_actions.update_actions_util.async_session_maker",
...
async def test_does_not_store_previous_image_after_rollback(mocker):
    ...
    await update_containers_data_after_execution(
        UpdatePlanResult(
            host_id=1,
            host_name="local",
            items=[_action_result("rolled_back")],
        )
    )
...
async def test_stores_previous_image_without_digests(mocker):
    ...
    await update_containers_data_after_execution(
        UpdatePlanResult(
            host_id=1,
            host_name="local",
            items=[_action_result("updated", tags=["my-app:latest"])],
        )
    )
