...
from app.helpers.get_setting_typed_value import get_setting_typed_value
...
router = APIRouter(
    prefix="/settings", tags=["settings"], dependencies=[Depends(is_authorized)]
)
...
async def get_settings(
    ...
    for s in settings:
        ...
        item = SettingsGetResponseItem(
            key=s.key, value=val, value_type=s.value_type, updated_at=s.updated_at
        )
        res.append(item)
...
async def change_system_settings(
    ...
    cron_expr = [item for item in request if item.key == ESettingKey.CRONTAB_EXPR][0]
    if cron_expr:
...
async def test_notification():
    await send_notification("Dockobserver", "This is test notification")
    return {}
